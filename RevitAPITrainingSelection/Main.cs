...
namespace RevitAPITrainingSelection
{
    ...
    public class Main : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            ////////////////IList<Reference> references = uidoc.Selection.PickObjects(ObjectType.Element, new PipeFilter(), "Выбор труб");
            ...
            ////////////////        double v1 = pipe.get_Parameter(BuiltInParameter.CURVE_ELEM_LENGTH).AsDouble();
            ////////////////        Parameter paramPipe = pipe.LookupParameter("Длина с коэффициентом");
            ////////////////        v1 = v1 * 1.1;
            ////////////////        paramPipe.Set(v1);
            ////////////////    }
            ////////////////    ts.Commit();
            ////////////////}
            ////////////////TaskDialog.Show("Сообщение \n", "Длина труб изменена с учетом коэффициента 1.1");
            ////////////////return Result.Succeeded;
            #endregion
            ...
            using (Transaction ts = new Transaction(doc, "Add parameter"))
            {
                ...
                {
                    double diamOuter1 = item.get_Parameter(BuiltInParameter.RBS_PIPE_OUTER_DIAMETER).AsDouble();
                    double diamOuter2 = UnitUtils.ConvertFromInternalUnits(diamOuter1, UnitTypeId.Millimeters);

                    double diamInner1 = item.get_Parameter(BuiltInParameter.RBS_PIPE_INNER_DIAM_PARAM).AsDouble();
                    double diamInner2 = UnitUtils.ConvertFromInternalUnits(diamInner1, UnitTypeId.Millimeters);
                    Parameter param = item.LookupParameter("Наменование");
                    param.Set(String.Format("Труба наружный диаметр {0} / внутренний диаметр {1}",diamOuter2,diamInner2));
                }
                ts.Commit();   
            }
            TaskDialog.Show("Сообщение! \n", "Диаметры труб изменены !");
            return Result.Succeeded;
